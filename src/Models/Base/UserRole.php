...
/**
 * Base class that represents a row from the 'xg_user_roles' table.
 *
 *
 *
...
abstract class UserRole implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\basteyy\\XzitGiggle\\Models\\Map\\UserRoleTableMap';
    ...
    /**
     * Initializes internal state of basteyy\XzitGiggle\Models\Base\UserRole object.
     */
    ...
    public function setIdentifier($v)
    {
        ...
        if ($this->identifier !== $v) {
            $this->identifier = $v;
            $this->modifiedColumns[UserRoleTableMap::COL_IDENTIFIER] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : UserRoleTableMap::translateFieldName('Identifier', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\basteyy\\XzitGiggle\\Models\\UserRole'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->userssScheduledForDeletion !== null) {
                if (!$this->userssScheduledForDeletion->isEmpty()) {
                    \basteyy\XzitGiggle\Models\UserQuery::create()
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if ($this->isColumnModified(UserRoleTableMap::COL_IDENTIFIER)) {
            $modifiedColumns[':p' . $index++]  = '`identifier`';
        }

        $sql = sprintf(
            'INSERT INTO `xg_user_roles` (%s) VALUES (%s)',
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(UserRoleTableMap::COL_IDENTIFIER)) {
            $criteria->add(UserRoleTableMap::COL_IDENTIFIER, $this->identifier);
        }
     ...
     * objects.
     *
     * @param object $copyObj An object of \basteyy\XzitGiggle\Models\UserRole (or compatible) type.
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \basteyy\XzitGiggle\Models\UserRole Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    ...
    public function initUserss(bool $overrideExisting = true): void
    {
        ...
        $this->collUserss->setModel('\basteyy\XzitGiggle\Models\User');
    }
        ...
        if (null === $this->collUserss || null !== $criteria || $partial) {
            if ($this->isNew()) {
                ...
                if (null === $this->collUserss) {
                    ...
                    $collUserss->setModel('\basteyy\XzitGiggle\Models\User');

                    return $collUserss;
                }
            } else {
                $collUserss = ChildUserQuery::create(null, $criteria)
     ...
     * @return $this The current object (for fluent API support)
     */
    public function addUsers(ChildUser $l)
    {
        if ($this->collUserss === null) {
            $this->initUserss();
            $this->collUserssPartial = true;
    ...
    /**
     * @param ChildUser $users The ChildUser object to add.
     */
    protected function doAddUsers(ChildUser $users): void
    {
    ...
    /**
     * @param ChildUser $users The ChildUser object to remove.
     * @return $this The current object (for fluent API support)
     */
    public function removeUsers(ChildUser $users)
    {
